...
class AvisoAdopcion(Base):
    ...
    comentarios = relationship("Comentario", back_populates="aviso", cascade="all, delete-orphan")  # comentarios asociados
...
class Comentario(Base):
    ...
    texto  = Column(String(300), nullable=False)  # match VARCHAR(300) de la BD
    fecha  = Column("fecha", DateTime, nullable=False, default=datetime.utcnow)
    aviso_id = Column(Integer, ForeignKey("aviso_adopcion.id"), nullable=False)
    # relacion hacia aviso
    aviso = relationship("AvisoAdopcion", back_populates="comentarios")

    def __repr__(self):
        return f"<Comentario id={self.id} aviso_id={self.aviso_id} nombre={self.nombre!r}>"
