...
'Nombre de la hoja Base en Repositorio.xlsx
...
'Hoja de configuración dentro del archivo del macro (Automation_Repo.xlsm)
'Config!B2 = ruta completa a Repositorio.xlsx (opcional)
'Config!B3 = nombre del UserForm (opcional)
...
'Input de Condor (celdas confirmadas)
...
'Output de Condor (hoja confirmada)
Private Const OUT_SHEET_NAME As String = "Results_IFRS4_Net"

'Series en output: "fila X a partir de la columna C"
Private Const OUT_START_COL As Long = 3 'C
Private Const OUT_ROW_WRITTEN_PREM_NET_TAX As Long = 24
Private Const OUT_ROW_DEVOLUCIONES As Long = 29
Private Const OUT_ROW_EP As Long = 39
Private Const OUT_ROW_CLAIMS As Long = 66

Public Sub abrir_rep_condor()
    'Carga un UserForm existente por nombre (si está configurado en Config!B3).
    'Si no está configurado o el nombre falla, ejecuta el export con prompts.
...
Public Sub exportar_rep_condor( _
    ...
    Optional ByVal observacion As String = vbNullString, _
    ...
    'Guardamos settings actuales para restaurar al final
    ...
    'Columnas en la tabla Base del repositorio (por header)
    ...
    'Se obtiene ruta del repositorio desde Config o mediante selector de archivo
    If Len(repoPath) = 0 Then repoPath = GetRepoPathFromConfigOrPicker()
    If Len(repoPath) = 0 Then Exit Sub

    'Se solicitan los archivos Condor si no vienen desde UserForm
    ...
    'Abrimos el input y el output de Condor
    ...
    Set wsOut = GetWorksheet(wbOut, OUT_SHEET_NAME)

    'ID interno = último ID en Base!A + 1
    nextId = NextIdFromBase(baseWS, REPO_ID_COL, REPO_HEADER_ROW + 1)

    'Lectura de celdas desde input de Condor
    ...
    'Lectura de series desde output de Condor
    ...
    'Siniestralidad preferible desde output si EP existe (claims/EP)
    If epTotal <> 0 Then
    ...
    'Devoluciones % sobre written premium net of tax
    ...
    'Buscamos columnas por header en la tabla Base del repositorio
    ...
    'Siguiente fila libre determinada por la última fila con ID
    ...
    'Campos opcionales (si vienen desde formulario)
    ...
    If col_DEV > 0 Then
        If Not IsEmpty(devolPct) Then baseWS.Cells(nextRow, col_DEV).Value2 = CDbl(devolPct)
    End If

    'Observación: se agrega trazabilidad de rutas y FX sin borrar lo que venga del formulario
    obsFinal = BuildObservation(observacion, condorInputPath, condorOutputPath, fxEURCLP)
...
Private Function BuildObservation(ByVal baseObs As String, ByVal inPath As String, ByVal outPath As String, ByVal fx As Double) As String
    Dim s As String
    s = Trim$(baseObs)
...
Private Function GetColByHeader(ws As Worksheet, headerName As String) As Long
    ...
    If c Is Nothing Then
        Err.Raise vbObjectError + 515, "GetColByHeader", _
            "No se encontró el header '" & headerName & "' en la fila " & REPO_HEADER_ROW & " de '" & ws.Name & "'."
    End If
